...
const VisibilityGraph = ({
  ...
  const addMidnightWeather = [
    ...todaysForecast,
    {
      vis_miles: todaysForecast[todaysForecast.length - 1].vis_miles,
    },
  ];
  ...
  const hourlyVisData = addMidnightWeather.map((hour, index) => ({
    hour: index,
    vis: hour.vis_miles,
    currentLineTop: index === currentHour ? 100 : undefined,
    currentLineBottom: index === currentHour ? 0 : undefined,
    currentPosition: index === xPosition ? hour.vis_miles : undefined,
  }));
    ...
    <View style={{ height: graphHeight }} className="relative z-0">
      ...
      >
        {({ points, chartBounds }) => {
          ...
          const leftPoints = points.vis.filter(
            (point) => (point.xValue! as number) <= cutoff
          ); // Left side (x <= 0)
          const rightPoints = points.vis.filter(
            (point) => (point.xValue! as number) >= cutoff
          ); // Right side (x >= 0)
          return (
            <>
              ...
              <>
                <Line
                  points={currentIndex === 0 ? rightPoints : points.vis}
              ...
              {isActive ? (
                <ToolTip x={state.x.position} y={state.y.vis.position} />
              ) : null}
            </>
          );
        }}
